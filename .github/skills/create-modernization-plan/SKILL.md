...
4. **Summary & Confirmation** (only when the `ask_user` tool is available):
...
5. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
...
6. **Rulebook Compliance Validation** (only when rulebook attachments are present):
